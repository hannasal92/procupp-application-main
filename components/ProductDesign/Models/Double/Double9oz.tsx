...
type GLTFResult = GLTF & {
  nodes: {
    cup_double_9oz: THREE.Mesh;
    cup_double_9oz_1: THREE.Mesh;
    cup_double_9oz_2: THREE.Mesh;
  };
  materials: {
...
export function Double9oz(props: JSX.IntrinsicElements["group"]) {
  const { nodes, materials } = useGLTF(
    "/product-design/models/double/9oz.glb"
  ) as GLTFResult;

  const globalState = useSnapshot(store);

  const [pos, setXYZ] = useState([0, 0.056, 0.024]);
  ...
  useEffect(() => {
    store.handleCenterArrow = () => setXYZ([0, 0.056, 0.024]);
  }, []);
        ...
        <group position={[0, 0.054, 0.04]}>
          <PivotControls
            ...
            onDrag={(local) => {
              ...
              setXYZ([position.x, position.y + 0.056, 0]);
...
useGLTF.preload("/product-design/models/double/9oz.glb");
