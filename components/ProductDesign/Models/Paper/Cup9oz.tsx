...
export function Cup9oz(props: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    "/product-design/models/paper/9oz.glb"
  ) as GLTFResult;

  const globalState = useSnapshot(store);

  const [pos, setXYZ] = useState([0, 0.053, 0.048]);
  const [scl, setScl] = useState([0.06, 0.06, 0.06]);
  ...
  useEffect(() => {
    store.handleCenterArrow = () => setXYZ([0, 0.053, 0.048]);
  }, []);
        ...
        <group position={[0, 0.05, 0.04]}>
          <PivotControls
            ...
            onDrag={(local) => {
              ...
              setXYZ([position.x, position.y + 0.053, 0]);
              setScl([0.06 * scale.y, 0.06 * scale.y, 0.06 * scale.z]);
            }}
          />
        </group>
...
useGLTF.preload("/product-design/models/paper/9oz.glb");
