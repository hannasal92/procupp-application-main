...
export function Cup4oz(props: JSX.IntrinsicElements["group"]) {
  const { nodes } = useGLTF(
    "/product-design/models/paper/4oz.glb"
  ) as GLTFResult;

  const globalState = useSnapshot(store);

  const [pos, setXYZ] = useState([0, 0.032, 0.03]);
  ...
  useEffect(() => {
    store.handleCenterArrow = () => setXYZ([0, 0.032, 0.03]);
  }, []);
        ...
        <group position={[0, 0.029, 0.04]}>
          <PivotControls
            ...
            onDrag={(local) => {
              ...
              setXYZ([position.x, position.y + 0.032, 0]);
...
useGLTF.preload("/product-design/models/paper/4oz.glb");
