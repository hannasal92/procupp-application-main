...
export function V1Large(props: JSX.IntrinsicElements["group"]) {
  const { nodes, materials } = useGLTF(
    "/product-design/models/plastic/v1_l.glb"
  ) as GLTFResult;

  const globalState = useSnapshot(store);

  const [pos, setXYZ] = useState([0, 0.078, 0.024]);
  const [scl, setScl] = useState([0.078, 0.078, 0.078]);
  ...
  useEffect(() => {
    store.handleCenterArrow = () => setXYZ([0, 0.078, 0.024]);
  }, []);
        ...
        <group position={[0, 0.075, 0.04]}>
          <PivotControls
            ...
            onDrag={(local) => {
              ...
              setXYZ([position.x, position.y + 0.078, 0]);
              setScl([0.078 * scale.y, 0.078 * scale.y, 0.078 * scale.z]);
            }}
          />
        </group>
...
useGLTF.preload("/product-design/models/plastic/v1_l.glb");
