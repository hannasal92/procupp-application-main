...
export function V2Small(props: JSX.IntrinsicElements["group"]) {
  const { nodes, materials } = useGLTF(
    "/product-design/models/plastic/v2_s.glb"
  ) as GLTFResult;

  const globalState = useSnapshot(store);

  const [pos, setXYZ] = useState([0, 0.05, 0.024]);
  ...
  useEffect(() => {
    store.handleCenterArrow = () => setXYZ([0, 0.05, 0.024]);
  }, []);
...
useGLTF.preload("/product-design/models/plastic/v2_s.glb");
